...
void Benchmark::bench_bitcount(int N)
{
	Rand r(123456789); 	
	Clock timer;
	U64 * bit_data = new U64[N];
	for (int i = 0; i < N; ++i)
	{
		bit_data[i] = r.rand64();
	}
	U64 ans1 = 0ULL; U64 ans2 = 0ULL;
	timer.start();
	for (int i = 0; i < N; ++i)
	{
		ans1 += builtin_popcount(bit_data[i]);
	}
	timer.stop();
	double builtin_ms = timer.ms();

	timer.start();
	for (int i = 0; i < N; ++i)
	{
		ans2 += count(bit_data[i]);
	}
	timer.stop();
	double standard_ms = timer.ms();
	std::cout << "builtin_popcount(" << N << ")\t" << std::right << std::setw(10) << builtin_ms << "ms " << ans1 
		<< "\tstandard_popcount(" << N << ")\t" << std::right << std::setw(10) << standard_ms << "ms " << ans2 <<std::endl;
}

void Benchmark::bench_lsb(int N)
{
	Rand r(123456789);
	Clock timer;
	U64 * bit_data = new U64[N];
	...
	timer.stop();
	double standard_ms = timer.ms();
	std::cout << "builtin_lsb(" << N << ")\t" << std::right << std::setw(10) << builtin_ms << "ms " << ans1
		<< "\tstandard_lsb(" << N << ")\t" << std::right << std::setw(10) << standard_ms << "ms " << ans2 << std::endl;
}
