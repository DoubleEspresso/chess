...
void Benchmark::init(BenchType bt) {
  ...
  else if (bt == BENCH) bench(max_iter);
}
...
void Benchmark::divide(Board& b, int d) {
  ...
  for (MoveGenerator mvs(b); !mvs.end(); ++mvs) {
    ...
    total += n;
    b.undo_move(mvs.move());
    std::cout << SanSquares[get_from(mvs.move())]
	      << SanSquares[get_to(mvs.move())]
	      << '\t' << n << std::endl;
  }
  std::cout << "---------------------------------" << std::endl;
...
void Benchmark::bench_bitcount(int N) {
  ...
  std::cout << "builtin_popcount(" << N << ")\t"
	    << std::right << std::setw(10)
	    << builtin_ms
	    << "ms "
	    << ans1 
	    << "\tstandard_popcount(" << N << ")\t" << std::right
	    << std::setw(10) << standard_ms << "ms "
	    << ans2 <<std::endl;
}
...
void Benchmark::bench_lsb(int N) {
  ...
  timer.stop();
  double standard_ms = timer.ms();
  std::cout << "builtin_lsb(" << N << ")\t"
	    << std::right << std::setw(10)
	    << builtin_ms << "ms " << ans1
	    << "\tstandard_lsb(" << N << ")\t"
	    << std::right << std::setw(10)
	    << standard_ms << "ms " << ans2 << std::endl;
}
