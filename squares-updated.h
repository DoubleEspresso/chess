...
namespace {
    ...
    {
     {
      ...
      0.257,  0.193,  0.377,  0.332,  0.481,  0.276,  0.169,  0.203, 
     ...
     
     {
      ...
      0.067,  0.020,  0.112,  0.235,  0.228,  0.147,  0.027,  0.047, 
      0.024,  0.077,  0.157,  0.200,  0.237,  0.153,  0.069,  0.018, 
     ...
     {
      // rooks
      0.000,  0.053,  0.070,  0.591,  0.391,  0.591,  0.046,  0.000, 
  ...
  template<> float square_score<black>(const Piece& p, const Square& s) {
    return 15 * sq_scores[p][56 - 8 * util::row(s) + util::col(s)];
  }
  
  
  template<> float square_score<white>(const Piece& p, const Square& s) {   
    return 15 * sq_scores[p][s];
  }  
}
