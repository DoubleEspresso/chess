...
bool Board::is_promotion(const U16& m)
{
	...
	return (type != MOVE_NONE && (type <= PROMOTION) || (type > PROMOTION && type <= PROMOTION_CAP));
}
...
{
	U64 stepper_checks = (PawnAttacksBB[fc][ks] & pieces[ec][PAWN] | PseudoAttacksBB(KNIGHT, ks) & pieces[ec][KNIGHT] | PseudoAttacksBB(KING, ks) & pieces[ec][KING]);
	if (stepper_checks) return true;
...
int Board::see2(const U16& move)
{
	int to = get_to(move); int f = get_from(move);
	//U64 fto = SquareBB[f] | SquareBB[to];
	U64 tmp = all_pieces(); U64 attackers = 0ULL;
	U64 white_bb = colored_pieces(WHITE); int widx = 0;
	U64 black_bb = colored_pieces(BLACK); int bidx = 0;
	SeePiece black_list[16]; SeePiece white_list[16]; // final storage   
	...
	//printf("..white attackers sorted: ");
	//for(int i=0; i<widx; ++i) printf("%d ", white_list[i].v);//SanPiece[white[i].p]);
	//printf("\n..black attackers sorted: ");
	//for(int i=0; i<bidx; ++i) printf("%d ", black_list[i].v);//SanPiece[black[i].p]);
	//printf("\n");
	...
	for(int i=0; i < 2*MIN(widx, bidx)-1; ++i)
	{
		...
		int attacker = (color == WHITE ? white_list[w].p : black_list[b].p);
		color ^= 1;
		//printf("i=%d, widx=%d, bidx=%d, victim=%d, attacker=%d, c=%d, score=%d\n", i, widx, bidx, victim, attacker, color, score);
		//if (victim >= 4 ) printf("%d\n", victim);
		int d = material.material_value(victim, MIDDLE_GAME);// - material.material_value(attacker, MIDDLE_GAME);
		score += (i%2 == 0 ? -d : d);
	}
	//if (widx < bidx) score += material.material_value(white_list[widx-1].p, MIDDLE_GAME);//for (int i=b; i<bidx; ++i) printf("..%d remains\n", black_list[i]);
	//if (bidx < widx) score += material.material_value(black_list[bidx-1].p, MIDDLE_GAME);//for (int i=w; i<widx; ++i) printf("..%d remains\n", white_list[i]);
	return score;
}
...
int Board::see_sign(U16& move)
{
	...
	return see2(move);//see_move(move);
}
