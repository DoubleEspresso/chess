...
namespace magics {
  ...
  template<Piece p> inline U64 attacks(const U64& occ, const int& s);

  template<> inline U64 attacks<Piece::rook>(const U64& occ, const int& s) {
    using namespace detail;
    return rattks[ridx[s][rtable[s].entry(occ)] + rtable[s].offset];
  }
  
  template<> inline U64 attacks<Piece::bishop>(const U64& occ, const int& s) {
    using namespace detail;
