...
// deprecated idea (didn't speed up code)
//U8 rook_tos[4900][14]; // storage for the hashed to-squares of the rook (instead of the full bitboard of pieces moves)
//U8 bish_tos[1428][13]; // storage for the hashed to-squares of the bishop (instead of the full bitboard of pieces moves)
	...
	for (int p = BISHOP; p <= ROOK; ++p)
	{
		...
		for (int s = A1; s <= H8; ++s)
		{
			...
			for (i = 0; i < occ_size; ++i)
			{
				...
				int o = (p == ROOK ? r_occ[s][idx] + offset : b_occ[s][idx] + offset);
				(p == ROOK ? rook_atks[o] = atks[i] : bish_atks[o] = atks[i]);

				// DBG hash the chopped bitboard (to squares) .. currently unused
				//U64 a = atks[i]; int k = 0; 
				//while (a)
				//{
				//	(p == ROOK ? rook_tos[o][k] = pop_lsb(a) : bish_tos[o][k] = pop_lsb(a));
				//	++k;
				//}
				//int max = (p == ROOK ? 14 : 13);
				//if (k < max) 
				//	for (; k < max; ++k)
				//		(p == ROOK ? rook_tos[o][k] = SQUARE_NONE : bish_tos[o][k] = SQUARE_NONE);
			}
			...
			if (p == BISHOP)
			{
				...
				//BMagics[s].tosqs = new U8*[1428];
				//for (int j = 0; j < 1428; ++j)
				//{
				//	BMagics[s].tosqs[j] = bish_tos[j];
				//};
			}
			else
			{
				...
				//RMagics[s].tosqs = new U8*[4900];
				//for (int j = 0; j < 4900; ++j)
				//{
				//	RMagics[s].tosqs[j] = rook_tos[j];
				//};
			}
		}
	}
